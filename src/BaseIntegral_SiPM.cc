...
void BaseIntegral_SiPM(std::string fileNum, std::string pedNum, enum runMode mode) {
    ...
    for (int j = 0; j < NofDAQModule; j++) { 
        ...
        if ( j == 0 ) {
            ...
            c->cd(); pad_LB->cd(); TLegend* leg3 = new TLegend(0.70,0.50,0.90,0.86); leg3->SetFillStyle(0); leg3->SetBorderSize(0); leg3->SetTextFont( 42 ); leg3->SetTextSize( 0.045 );
            c->cd(); pad_RB->cd(); TLegend* leg4 = new TLegend(0.70,0.50,0.90,0.86); leg4->SetFillStyle(0); leg4->SetBorderSize(0); leg4->SetTextFont( 42 ); leg4->SetTextSize( 0.045 );
            for (int i = 0; i < 8; i++) {
                if ( i == 0 ) {
        ...
        if ( j == 1 ) {
            ...
            for (int i = 0; i < chInput.at(j).size(); i++) {
                c1->cd();
                if ( i == 0 ) {
                    plotCol.at(j).at(i)->Draw("Hist"); leg1->AddEntry(plotCol.at(j).at(i), plotName.at(i), "l");
                } else {
                    plotCol.at(j).at(i)->Draw("Hist&sames"); leg1->AddEntry(plotCol.at(j).at(i), plotName.at(i), "l");
                }
